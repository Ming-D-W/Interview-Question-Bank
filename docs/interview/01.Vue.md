...
- hook
  ...
  hooks 推崇函数式编程思想，允许开发者用纯函数的形式书写组件逻辑，那这样的逻辑呢就更加的简洁可预测

  逻辑复用是 hooks 提供的一种机制，通过自定义 hook函数把组件间共享的逻辑提取出来便于在多个组件间复用，这就消除了以往 mixin 带来的嵌套作用域和命名冲突问题

### ref和reactive

reactive是 Vue3 为引用类型（对象 / 数组）设计的响应式 API，其底层通过Proxy代理整个对象，拦截对象的属性读取、修改、删除等操作，从而实现响应式。Proxy只能代理对象（引用类型），对基本类型无法拦截其赋值操作 —— 因为基本类型的赋值是直接替换值，而非修改属性，Proxy无法感知这种变化。

ref的设计初衷就是解决reactive无法处理基本类型的问题，它通过包装对象的方式，让基本类型拥有响应式能力。通过get/set拦截value属性的访问和修改，从而实现响应式 —— 这也是为什么访问ref值需要加.value的原因。当 ref 包裹对象的时候是创建 reactive

### pinia Vuex

| Vuex                                                         | pinia                                                        |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| API 设计严格区分 state（状态）、mutations（同步修改）、actions（异步/复杂逻辑）、getters（计算属性），强制通过 mutations 修改状态 | 取消 mutations，仅保留 state、actions、getters；支持直接修改状态（或通过 actions），API 更简洁。 |
...
vuex的数据流程，简单描述一下：就是组件通过调用 `dispatch` 触发一个 Action，Action 的处理函数执行一些异步操作，然后提交一个 Mutation，Mutation 的处理函数修改 State，State 的变化触发视图的更新
...
`router-link`是`vue-router`提供的一个组件，用来创建类似于HTML中a标签的导航链接。使用`router-link`可以声明式地定义路由跳转，它会渲染成一个a元素，并且自动处理URL的hash值或history模式下的路径。
...
在上面的例子中，第一个*router-link*通过*to*属性指定了跳转的路径，第二个*router-link*则通过命名路由和参数来定义跳转。

**编程式导航：`this.$router.push` 和 `this.$router.replace`**

`this.$router.push`方法用于导航到新的URL，而`this.$router.replace`则用于替换当前URL，不会在浏览器的历史记录中留下记录。

### 路由传参方式

在进行路由跳转时，有时需要向目标组件传递参数。`vue-router`提供了两种传参方式：`query`和`params`。`query`方式类似于HTTP的GET请求，参数会显示在URL中。`params`方式则类似于POST请求，参数不会显示在URL中。
...
- MVVM
  ...
  VM： ViewModel是连接View和Model的中间件，实现视图和数据模型的通信
...
### Vue中某一个响应式属性的值发生改变后，视图会立即同步执行重新渲染吗？

Vue 中某一个响应式属性的值发生改变后，视图不会立即同步执行重新渲染。

从响应式原理来看，Vue采用的是异步更新策略，具体流程如下：

1. **数据变化检测**：当data中的属性值发生改变时，会触发通过Object.defineProperty(Vue 2)或Proxy(Vue 3)设置的setter方法。
2. **依赖通知**：setter方法会调用dep.notify()通知所有订阅了该数据变化的Watcher实例。
3. **异步队列机制**：Vue不会立即执行DOM更新，而是将需要更新的Watcher推入一个异步队列中。Vue会在同一个事件循环中收集所有需要进行的更新。
4. **去重处理**：如果同一个Watcher被多次触发，只会被推入队列一次，避免不必要的计算和DOM操作。
5. **批量更新**：在下一个事件循环的"tick"中，Vue会刷新队列并执行实际(已去重的)工作，完成DOM更新。
...
如果需要在数据变化后立即获取更新后的DOM，可以使用`Vue.nextTick()`或实例方法`this.$nextTick()`，它会在DOM更新完成后执行回调。
...
**nextTick的实际用途**：nextTick主要用于以下场景：

- 在数据变化后立即操作刚刚更新的DOM元素
- 在数据变化后获取更新后的DOM尺寸或位置信息
- 在数据变化后与第三方DOM库交互
- 在测试中等待DOM更新完成后再进行断言
...
- beforeUnmount（Vue2: beforeDestroy）: 即将卸载

- unmounted（Vue2: destroyed）: 组件卸载完成
  - 应用场景：清除挂载在window相关的行为，例如定时器、事件监听
