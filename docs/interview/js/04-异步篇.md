...
JavaScript是单线程的，通过事件循环（Event Loop）机制来处理异步任务。JavaScript引擎会逐行解析和执行代码，遇到同步代码立即执行，遇到异步代码会执行他的同步部分并将它的回调函数推到任务队列中。当前执行栈清空之后，会去检查任务队列，先看微任务队列，如果有，就挨个推到执行栈执行，直到微任务队列清空。在执行一个优先级最高的宏任务，再去看微任务队列，重复这个过程直到执行栈和任务队列都为空
