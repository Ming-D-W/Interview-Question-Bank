...
### bfc（概念，作用，哪些能够触发，应用场景，解决问题）

   >`BFC`（Block Formatting Context），即块级格式化上下文，它是页面中的一块渲染区域，一个相对于外界完全独立的空间，让内部的子元素不会影响到外部的元素
   >
   >触发bfc: 
   >
   >position为absolute或fixed display为inline-block, table-cell, table-caption, flex, inline-flex overflow不为visible
   >bfc的应用:
   >外边距重叠: 触发bfc
   >浮动导致高度丢失: 触发bfc
   >清除浮动: 触发bfc
